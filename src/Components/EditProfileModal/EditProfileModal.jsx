...
const EditProfileModal = ({ open, setOpen, handleOpen, handleClose }) => {
 ...
 const [fullname, setFullname] = useState(workerData?.fullname)
...
async function EditHandler(e){
    ...
    try {
        ...
        formData.append('file', file ? file : workerData?.img)
        const { data } = await axios.put(`${BACKEND_URL}/api/users/edit-my-profile`, formData, { headers : { token : workerData?.token } })
        if(data.success){
            toast.success(data.message);
            localStorage.setItem('workerData', JSON.stringify({...data.data, token : workerData?.token }))
            location.reload();
        }else{
  ...
  >
    <Box sx={style}>
      ...
      <Container component="main" maxWidth="xs">
        ...
        >
           <Avatar sx={{ m: 1,  }}> 
           <img src={`${BACKEND_URL}/${workerData?.img}`}/>
           </Avatar> 
